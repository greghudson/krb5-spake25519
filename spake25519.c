...
SPAKE2_CTX *SPAKE2_CTX_new(enum spake2_role_t my_role) {
  SPAKE2_CTX *ctx = malloc(sizeof(SPAKE2_CTX));
  if (ctx == NULL) {
    return NULL;
  }
...
int SPAKE2_generate_msg(SPAKE2_CTX *ctx, uint8_t *out, const uint8_t *w) {
  ...
  /* XXX generate 64-bit w value instead */
  uint8_t wtmp[SHA512_DIGEST_LENGTH];
  memcpy(ctx->password_hash, w, sizeof(ctx->password_hash));
  memcpy(wtmp, w, sizeof(wtmp));
  x25519_sc_reduce(wtmp);
  memcpy(ctx->password_scalar, wtmp, sizeof(ctx->password_scalar));
...
int SPAKE2_process_msg(SPAKE2_CTX *ctx, uint8_t *out,
  ...
  memcpy(out, dh_shared_encoded, 32);
...
{
    uint8_t w[64], pub1[32], pub2[32], key1[32], key2[32];
    SPAKE2_CTX *ctx1, *ctx2;
    int i, iter = atoi(argv[1]);

    ctx1 = SPAKE2_CTX_new(spake2_role_alice);
    ctx2 = SPAKE2_CTX_new(spake2_role_bob);
    memset(w, 0, sizeof(w));
    for (i = 0; i < iter; i++) {
	SPAKE2_generate_msg(ctx1, pub1, w);
	SPAKE2_generate_msg(ctx2, pub2, w);
	SPAKE2_process_msg(ctx1, key1, pub2);
	SPAKE2_process_msg(ctx2, key2, pub1);
    }
    return 0;
}
